...
hJyNbhBjHJBiHmhO.BJJvItJgJvnKnuOuIRWfTF.chk();
		...
		infoSHhit = new FlxText(healthBar.x, healthBar.y + 18, 0, "Score: 0 | Combo Breaks: 0");
		infoSHhit.scrollFactor.set();
		infoSHhit.setFormat("VCR OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(infoSHhit);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var ntype = songNotes[3];
				var swagNote:Note = new Note(daStrumTime, daNoteData, ntype, oldNote);
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, ntype, oldNote, true);
	...
	override public function update(elapsed:Float)
	{
		...
		else if (misses == 0 && goods > 0 && bads == 0 && shits == 0)
		{
			rating = "GFC";
		}
		...
		// literally copied from psych EZ
		...
		var timeleft:String = FlxStringUtil.formatTime(secondsTotal, false);
		infoSHhit.text = 'Score: $songScore | Combo Breaks: $misses | Time left: $timeleft | Rating: $rating';

		notes.forEachAlive(function(daNote:Note)
		{
			if (daNote.y < -FlxG.height && daNote.mustPress)
			{
				daNote.active = false;
				daNote.visible = false;
				switch (CoolUtil.Instance.noteLabelFromPrefix(daNote.noteType))
				{
					case "Hurt":
						// meant to miss it, so don't do anything
					default:
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			if (!note.isSustainNote)
			{
				...
				switch (CoolUtil.Instance.noteLabelFromPrefix(note.noteType))
				{
					case "Hurt":
						health -= 0.1;
					default:
