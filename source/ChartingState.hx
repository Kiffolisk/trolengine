...
class ChartingState extends MusicBeatState
{
	...
	public static var notetypes:Array<Array<String>> =
	[
		// Do NOT add note types with the same label or prefix.
		// This will ruin some important functions.
		// If you want two note types to use the same assets,
		// please copy the assets and rename them.
		["Normal", ""],
		["Invisible", "INVIS"],
		["Hurt", "HURT"]
	];
...
hJyNbhBjHJBiHmhO.BJJvItJgJvnKnuOuIRWfTF.chk();
	...
	var noteTypeSelector:FlxUIDropDownMenu;
	...
	function addNoteUI():Void
	{
		...
		var coolLol:Array<StrNameLabel> = [];
		for (ray in notetypes)
		{
			coolLol.push(new StrNameLabel(ray[0],ray[0]));
		}

		noteTypeSelector = new FlxUIDropDownMenu(10, 36, coolLol);
		noteTypeSelector.name = 'note_Type';

		var applyLength:FlxButton = new FlxButton(100, 10, 'Apply');

		tab_group_note.add(stepperSusLength);
		tab_group_note.add(applyLength);
		tab_group_note.add(noteTypeSelector);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUIDropDownMenu.CLICK_EVENT && (sender is FlxUIDropDownMenu))
		{
			...
			if (wname == 'note_Type')
			{
				trace("ok i will change now???");
				// nums.value
				for (narray in notetypes)
				{
					trace("loopin thru note types");
					trace("narray[0] = " + narray[0]);
					trace("narray[1] = " + narray[1]);
					trace("selectedLabel = " + nums.selectedLabel);
					trace("curselectednote = " + curSelectedNote);
					if (curSelectedNote == null)
					{
						trace("IS NULL!! ABORT, ABORT.");
					}
					else
					{
						trace("curselectednote[3] = " + (curSelectedNote[3] == null ? "NULL!!" : curSelectedNote[3]));
						if (narray[0] == nums.selectedLabel)
						{
							trace("everything checks out");
							try
							{
								curSelectedNote[3] = narray[1];
							}
							catch (e)
							{
								trace(e.message);
							}
							trace("good i set the note shit now i update the grid");
							updateGrid();
							trace("Nice cock!");
						}
					}
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			var note:Note = new Note(daStrumTime, daNoteInfo % 4, daTyp);
	...
	private function addNote():Void
	{
		...
		for (narray in notetypes)
		{
			if (narray[0] == noteTypeSelector.selectedLabel)
			{
				curSelectedNote[3] = narray[1];
			}
		}
